...
const API_URL = 'https://proyecto-moduloseguridad.onrender.com/api/auth/reset-password/';
  ...
  try {
    ...
    try {
      res = await fetch(API_URL, {
        ...
        body: JSON.stringify({ email, codigo, new_password }),
    ...
    if (!res.ok || data?.success === false) {
      return NextResponse.json(
        { success: false, message: data?.message || data?.error || 'Error al cambiar la contraseña' },
    ...
    return NextResponse.json({
      success: true,
      message: data?.message || 'Contraseña actualizada con éxito',
    });
  } catch (error: any) {
